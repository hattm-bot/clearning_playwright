# MEMORY — Source Code Context

> File này tracking cấu trúc source code automation.
> Tách biệt với 02_analyze-requirements/[version]/MEMORY.md (tracking requirement analysis).
> Cập nhật lần cuối: 2026-06-28

## 1. Project Structure
> Chạy skill scan-source-code để tự động điền section này.
>
> ⚠️ Lưu ý: Source Playwright hiện tại đang nằm ở **root của repo** (không phải trong 10_source-code/):
> `playwright.config.ts`, `tests/` (vd `tests/example.spec.ts`), `playwright-report/`, `test-results/`.
> Có thể chạy `/scan-source-code` (point vào root) hoặc `/init-source-code --archetype playwright-ts`
> để scaffold lại cấu trúc POM chuẩn trong thư mục này.

```
(root)/
├── playwright.config.ts
├── package.json
├── tests/
│   └── example.spec.ts
├── playwright-report/
└── test-results/

10_source-code/
└── MEMORY.md   (file này — tracking)
```

## 2. Tech Stack
| Component | Version | Ghi chú |
|-----------|---------|---------|
| Framework | Playwright + TypeScript | Test runner @playwright/test |
| Language | TypeScript | |
| Config | playwright.config.ts (root) | |

> Chạy scan-source-code để extract versions chính xác từ package.json / package-lock.json.

## 3. Config
> Sẽ được extract tự động từ source code. (playwright.config.ts ở root)

## 4. Base Classes
> Sẽ được extract tự động.

## 5. Conventions
> Sẽ được extract tự động từ source code hiện có khi chạy scan-source-code.

## 6. Page Classes Registry
| Page Class | File Path | Elements | Methods | Scenarios Cover | Last Updated |
|------------|-----------|----------|---------|-----------------|-------------|

## 7. Test Classes Registry
| Test Class | File Path | Test Methods | Scenarios Cover | Status |
|------------|-----------|-------------|-----------------|--------|

## 8. Utilities
> Sẽ được extract tự động.

## 9. Coverage Gap
> Sẽ được tạo bởi scan-source-code.

## 10. Scan History
| Ngày | Mode | Scope | Ghi chú |
|------|------|-------|---------|

## 11. Issues
| # | Issue | Severity | Status |
|---|-------|----------|--------|

## 12. Locator Registry
> Sử dụng MCP Playwright để lấy locator từ web thực.

| Element | Strategy | Value | Page | Date |
|---------|----------|-------|------|------|

## 13. Implementation Log
| TC ID | SC ID | Test Method | Steps Mapped | Expected Mapped | Manual Verify | Member | Date |
|-------|-------|-------------|-------------|----------------|--------------|--------|------|

## 14. Locator Issues
| Element | Page | Issue | Status | Ngày phát hiện |
|---------|------|-------|--------|---------------|

## 15. Execution Log
| Run ID | Date | Version | Scope | Command | Total | Pass | Fail | Skip | Pass Rate |
|--------|------|---------|-------|---------|-------|------|------|------|-----------|

## 16. Fail Registry
| Fail ID | Run ID | Date | Version | Class.Method | SC ID | Fail Type | Error | Status | Bug ID |
|---------|--------|------|---------|-------------|-------|-----------|-------|--------|--------|

## 17. SRC-TC Review
| Date | Version | Mode | Score | Step Cov | Assert Cov | Verdict |
|------|---------|------|-------|----------|------------|---------|
